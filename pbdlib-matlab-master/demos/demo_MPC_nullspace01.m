...
% u1 = pinvJ * U' * (MuQ - Sx * x0); %Principal task
...
u = u1; %Least squares solution
x = reshape(Sx*x0+Su*u, nbVar, nbData); %Reshape data for plotting

%General solutions
for n=1:nbRepros
	w = randn(nbVarPos,nbRBF) .* 1E0; %Random weights
	u2 = w * H; %Reconstruction of signals by weighted superposition of basis functions
	uTmp = u1 + N * u2(:);
...
axis equal; 
% print('-dpng','graphs/demo_MPC_nullspace01.png');
