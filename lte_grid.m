...
for i = 1:grid_size
    for j = 1:grid_size
        ...
        %plot(x, y, lte_deformed_x{i, j}, lte_deformed_y{i, j});
        %pause;
...
%     for j = 1:grid_size
%         plot(grid_x(i), grid_y(j), 'k+');
%         plot(lte_deformed_x{i, j}, lte_deformed_y{i, j});
%     end
% end
% plot(x, y, 'r');
...
for k = 1:i * j
    ...
    a = constrain(a, 1, grid_size);
    b = constrain(b, 1, grid_size);
    subplot(grid_size, grid_size, k);
    plot(x, y, lte_deformed_x{b, a}, lte_deformed_y{b, a});
end
% figure;
% for k = 1:i * j
%     a = floor(k / i) + 1;
