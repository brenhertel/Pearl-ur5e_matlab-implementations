...
class mlfd(object):
  ...
  def create_grid_old(self, given_grid_size, dists, disp=False):
    ...
    if (self.n_dims >= 1):
        ...
        if self.mode == 'end_point':
            center = point(self.org_x[-1])
    ...
    if (self.n_dims >= 2):
        ...
        if self.mode == 'end_point':
            center = point(self.org_x[-1], self.org_y[-1])
    ...
    if (self.n_dims >= 3):
        ...
        if self.mode == 'end_point':
            center = point(self.org_x[-1], self.org_y[-1], self.org_z[-1])
    ...
    for m in range (self.n_algs):
        if self.n_dims == 1:
            for i in range (self.grid_size):
                ...
                if self.mode == 'end_point':
                    self.grid_deforms_x[i][m].traj = self.algs[m](self.org_x, end=self.grid[i].x)
                if plot == True:
        ...
        if self.n_dims == 2:
            for i in range (self.grid_size):
                for j in range (self.grid_size):
                    ...
                    if self.mode == 'end_point':
                        self.grid_deforms_x[i][j][m].traj = self.algs[m](self.org_x, end=self.grid[i][j].x)
                        self.grid_deforms_y[i][j][m].traj = self.algs[m](self.org_y, end=self.grid[i][j].y)
            ...
            for i in range (self.grid_size):
                for j in range (self.grid_size):
                    for k in range (self.grid_size):
                        ...
                        if self.mode == 'end_point':
                            self.grid_deforms_x[i][j][k][m].traj = self.algs[m](self.org_x, end=self.grid[i][j][k].x)
                            self.grid_deforms_y[i][j][k][m].traj = self.algs[m](self.org_y, end=self.grid[i][j][k].y)
                            self.grid_deforms_z[i][j][k][m].traj = self.algs[m](self.org_z, end=self.grid[i][j][k].z)
    ...
    if self.n_dims == 2:
        for i in range (self.grid_size):
            for j in range (self.grid_size):
                ...
                X[(j * self.grid_size) + (i)][0] = self.x_vals[i]
                X[(j * self.grid_size) + (i)][1] = self.y_vals[j]
                Y[(j * self.grid_size) + (i)] = A[i][j]
    if self.n_dims == 3:
        for i in range (self.grid_size):
            for j in range (self.grid_size):
                for k in range (self.grid_size):
                    X[(k * self.grid_size**2) + (j * self.grid_size) + (i)][0] = self.x_vals[i]
                    X[(k * self.grid_size**2) + (j * self.grid_size) + (i)][1] = self.y_vals[j]
                    X[(k * self.grid_size**2) + (j * self.grid_size) + (i)][2] = self.z_vals[k]
  ...
  def reproduce_optimal_at_point(self, coords, plot=False, mode='save', filepath=''):
    ...
    if self.n_dims == 2:
        ...
        if self.mode == 'end_point':
            opt_x = opt_alg(self.org_x, end=coords[0][0])
            opt_y = opt_alg(self.org_y, end=coords[0][1])
  ...
  def generate_svm_region(self, mode='save', filepath=''):
    ...
    if self.n_dims == 2:
        ...
        plt.plot(self.org_x[-1], self.org_y[-1], 'k.')
        plt.xticks(self.x_vals)
    ...
    if self.n_dims == 2:
        ...
        #for t in range (len(xnew)):
            #for u in range (len(ynew)):
                ...
                #print(self.clf.predict(coords))
        #        plt.plot(xnew[t], ynew[u], colors[int(self.clf.predict(coords))] + '.')
        plt.plot(self.org_x[-1], self.org_y[-1], 'k.', markersize=30)
        if plot_point != None:
