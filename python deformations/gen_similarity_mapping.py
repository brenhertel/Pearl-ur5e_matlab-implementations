...
def gsm(x_data, y_data, name='', is_dmp_on=False, grid_size=5, grid_x_dist=-1.0, grid_y_dist=-1.0):
    ...
    #gradient maps
    gradient_plotting.gradient_map(fd_lte, name + ' LTE Frechet Distance', plt_fpath)
    gradient_plotting.gradient_map(hd_lte, name + ' LTE Haussdorf Distance', plt_fpath)
    gradient_plotting.gradient_map(fd_ja, name + ' JA Frechet Distance', plt_fpath)
    gradient_plotting.gradient_map(hd_ja, name + ' JA Haussdorf Distance', plt_fpath)
    if is_dmp_on:
        gradient_plotting.gradient_map(fd_dmp, name + ' DMP Frechet Distance', plt_fpath)
        gradient_plotting.gradient_map(hd_dmp, name + ' DMP Haussdorf Distance', plt_fpath)
        gradient_plotting.rgb_gradient(fd_ja, fd_lte, fd_dmp, name=(name + ' Frechet Distance Compared Reproductions'), plt_fpath)
        gradient_plotting.rgb_gradient(hd_ja, hd_lte, hd_dmp, name=(name + ' Haussdorf Distance Compared Reproductions'), plt_fpath)
        gradient_plotting.strongest_gradient(fd_ja, fd_lte, fd_dmp, name=(name + ' Frechet Distance Best Reproductions'), plt_fpath)
        gradient_plotting.strongest_gradient(hd_ja, hd_lte, hd_dmp, name=(name + ' Haussdorf Distance Best Reproductions'), plt_fpath)
    else:
        gradient_plotting.rgb_gradient(fd_ja, fd_lte, np.zeros((np.shape(fd_lte))), name=(name + ' Frechet Distance Compared Reproductions'), plt_fpath)
        gradient_plotting.rgb_gradient(hd_ja, hd_lte, np.zeros((np.shape(fd_lte))), name=(name + ' Haussdorf Distance Compared Reproductions'), plt_fpath)
        gradient_plotting.strongest_gradient(fd_ja, fd_lte, np.zeros((np.shape(fd_lte))), name=(name + ' Frechet Distance Best Reproductions'), plt_fpath)
        gradient_plotting.strongest_gradient(hd_ja, hd_lte, np.zeros((np.shape(fd_lte))), name=(name + ' Haussdorf Distance Best Reproductions'), plt_fpath)
